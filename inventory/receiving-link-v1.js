...
async function enhanceReport(){
 ...
 try{const d=await post(RAPI,'inventory_totals',{date}),tot=d.totals||{};let linked=0;incoming.forEach(inp=>{const id=inp.dataset.id,total=Number(tot[id]||0),parent=inp.parentElement;if(total>0){linked++;if(Number(inp.value)!==total){inp.value=fmt(total);inp.dispatchEvent(new Event('input',{bubbles:true}))}inp.readOnly=true;inp.classList.add('auto-incoming');inp.dataset.morningChecked='1';let note=parent?.querySelector('.morning-linked');if(!note&&parent){note=document.createElement('small');note.className='morning-linked';parent.appendChild(note)}if(note)note.textContent='من الاستلام الصباحي'}else{inp.readOnly=false;inp.classList.remove('auto-incoming');inp.dataset.morningChecked='1';parent?.querySelector('.morning-linked')?.remove()}});
  let banner=q('.morning-link-banner');if(linked){if(!banner){banner=document.createElement('div');banner.className='morning-link-banner';const anchor=q('.note')||q('.section');anchor?.parentElement?.insertBefore(banner,anchor)}if(banner)banner.textContent=`تم ربط ${linked} صنف تلقائيًا باستلامات الصباح لهذا اليوم.`}else banner?.remove();
 }catch(e){}
 return true
}
